...
/* === NEJ-KNAPP: FRITT MEN LÅST INOM SKÄRMEN === */
const btn = document.getElementById("nejBtn");
...
function clampToScreen() {
  const r = btn.getBoundingClientRect();
  x = Math.max(padding, Math.min(x, window.innerWidth - r.width - padding));
  y = Math.max(padding, Math.min(y, window.innerHeight - r.height - padding));
}
